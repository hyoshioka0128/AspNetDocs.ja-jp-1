...
変更を行った後、保存ボタン (フロッピー ディスクのアイコン) をクリックして、エンティティ データ モデルを保存することを忘れないでください。 エンティティ デザイナーは、背後で Visual Basic .NET クラスのセットを生成します。 これらのクラスを表示するには、ソリューション エクスプローラー ウィンドウから MoviesDBModel.Designer.vb ファイルを開きます。

次にエンティティ デザイナーを使用するときに変更が上書きされるため、Designer.vb ファイルのコードは変更しないでください。 Designer.vb ファイルで定義されているエンティティークラスの機能を拡張する場合は、*部分クラス*を別々のファイルに作成できます。
...
db\_フィールドは、Movies データベーステーブルからレコードを取得するために、Index() アクション内で使用されます。 式\_db。ムービーセットは、ムービーデータベーステーブルからのすべてのレコードを表します。 ToList() メソッドは、ムービーのセットをムービーオブジェクトの汎用コレクションに変換するために使用されます: リスト(ムービーの)。
...
リスト 2 のビューには **、** 各ムービー レコードを反復処理し、ムービー レコードのタイトルとディレクターのプロパティの値を表示する For Each ループが含まれています。 各レコードの横に [編集] と [削除] リンクが表示されます。 さらに、ビューの下部に [ムービーの追加] リンクが表示されます (図 7 参照)。

**図 7 – インデックス ビュー**

![clip_image014](creating-model-classes-with-the-entity-framework-vb/_static/image7.jpg)

インデックス ビューは *、型付きビュー*です。 インデックス ビューには、&lt;継承属性を含&gt;む %@ ページ % ディレクティブがあります。 Inherits 属性は、ムービー オブジェクトの厳密に型指定された一般的なリスト コレクションに ViewData.Model プロパティをキャストするリスト (ムービーの) です。
...
**リスト 3 - コントローラー\ホームコントローラー.vb (メソッドの追加)**
...
**リスト 4 - コントローラー\ホームコントローラー.vb (メソッドの編集)**
...
**リスト 6 -- \コントローラー\ホームコントローラー.vb (削除アクション)**
